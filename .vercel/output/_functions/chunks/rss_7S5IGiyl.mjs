...
import { _t as AstroError, c as isRemotePath, d as removeBase, pt as UnknownContentCollectionError } from "./path_BVS9cGLr.mjs";
import { C as unescapeHTML, E as createComponent, f as renderTemplate, h as generateCspDigest, t as spreadAttributes } from "./server_CR6e1SMo.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_APSZSf_0.mjs");
...
function createGetCollection({ liveCollections }) {
	return async function getCollection(collection, filter) {
		...
		if (store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_u1KK2p_z.mjs");
			const result = [];
